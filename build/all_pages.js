...
(function () {
    ...
    var healthAdmin = angular.module('healthAdmin', ["ui.router"]);
    ...
    healthAdmin.config(['$httpProvider', '$stateProvider', '$urlRouterProvider', 'apiProvider', function ($httpProvider, $stateProvider, $urlRouterProvider, apiProvider) {
        ...
        $stateProvider.state("login", { //  用户管理=》医生
            url: "/login",
...
(function () {
    ...
    var app = angular.module('healthAdmin');
    ...
    app.provider('api', [function () {
        ...
        this.$get = ['$http', '$q', 'common', function ($http, $q, common) {
            ...
            var httpExecute = function httpExecute(url, method, data) {
                return $http({
            ...
            var httpDelete = function httpDelete(url) {
                return httpExecute(url, 'delete', {});
            };
            var httpPut = function httpPut(url, data) {
                return httpExecute(url, 'put', data);
            };
            var httpGet = function httpGet(url, data) {
            ...
            var getDoctorList = function getDoctorList(filterData, currentPage, pageSize) {
                var skip = ((currentPage || 1) - 1) * pageSize;
                var filterArray = [];
                var url = '/api/user/GetDoctors?$inlinecount=allpages&$orderby=RegisterDate desc&$top=' + pageSize + '&$skip=' + skip;
                if (filterData.startTime) {
                    var startDate = common.getDate(filterData.startTime);
                    filterArray.push("RegisterDate ge  datetime'" + startDate + "T00:00:00.0Z' ");
                }
                if (filterData.endTime) {
                    var endTime = new Date(filterData.endTime);
                    endTime.setDate(endTime.getDate() + 1);
                    var endtDate = common.getDate(endTime);
                    filterArray.push("RegisterDate le datetime'" + endtDate + "T00:00:00.0Z'");
                }
                if (filterData.keywords) {
                    var keywords = filterData.keywords;
            ...
            var getPatientList = function getPatientList(filterData, currentPage, pageSize) {
                var skip = ((currentPage || 1) - 1) * pageSize;
                var filterArray = [];
                var url = '/api/user/GetPatients?$inlinecount=allpages&$orderby=RegisterDate desc&$top=' + pageSize + '&$skip=' + skip;
                if (filterData.startTime) {
                    var startDate = common.getDate(filterData.startTime);
                    filterArray.push("RegisterDate ge  datetime'" + startDate + "T00:00:00.0Z' ");
                }
            ...
            var getExchangeList = function getExchangeList(filterData, currentPage, pageSize) {
                var skip = ((currentPage || 1) - 1) * pageSize;
                var filterArray = [];
                var url = '/api/Finance/ReviewExchange?$inlinecount=allpages&$orderby=CreateTime desc&$top=' + pageSize + '&$skip=' + skip;
                if (filterData.startTime) {
                    var startDate = common.getDate(filterData.startTime);
            ...
            var getOrderList = function getOrderList(filterData, currentPage, pageSize) {
                var skip = ((currentPage || 1) - 1) * pageSize;
                var filterArray = [];
                var url = '/api/BackOrder/GetAllOrders?$inlinecount=allpages&$orderby=OrderDate desc&$top=' + pageSize + '&$skip=' + skip;
            ...
            var putExchange = function putExchange(id, status, reason) {
                var url = '/api/Finance/Review/' + id;
                return httpPut(url, { Status: status, Reason: reason });
            };
            var getCoinDetailList = function getCoinDetailList(doctorId, filterStatus, currentPage, pageSize) {
                var skip = ((currentPage || 1) - 1) * pageSize;
                var url = 'api/CoinDetail/List?$inlinecount=allpages&$orderby=CreateTime desc&$top=' + pageSize + '&$skip=' + skip + "&userId=" + doctorId;
                if (filterStatus) {
                    url += filterStatus;
                }
                return httpGet(url, {});
            };
            var getBatchList = function getBatchList(filterData, currentPage, pageSize) {
                var skip = ((currentPage || 1) - 1) * pageSize;
                var filterArray = [];
                var url = '/api/Finance/Batches?$inlinecount=allpages&$orderby=CreateTime desc&$top=' + pageSize + '&$skip=' + skip;
                if (filterData.startTime) {
                    var startDate = common.getDate(filterData.startTime);
                    filterArray.push("CreateTime ge  datetime'" + startDate + "T00:00:00.0Z' ");
                }
            ...
            var getExchangePaymentBatch = function getExchangePaymentBatch(id) {
                var url = '/api/Finance/Batch/' + id;
                return httpGet(url, {});
            };
            var createExchangePaymentBatch = function createExchangePaymentBatch(ids) {
                var url = '/api/Finance/ExchangePaymentBatch';
                return httpPost(url, ids);
            };
            var getConsultings = function getConsultings(filterData, currentPage, pageSize) {
                var skip = ((currentPage || 1) - 1) * pageSize;
                var filterArray = [];
                var url = '/api/ConsultingAdmin/list?$inlinecount=allpages&$orderby=CreateTime desc&$top=' + pageSize + '&$skip=' + skip;
                if (filterData.startTime) {
                    var startDate = common.getDate(filterData.startTime);
                    filterArray.push("CreateTime ge  datetime'" + startDate + "T00:00:00.0Z' ");
                }
            ...
            var getConsulting = function getConsulting(id) {
                var url = '/api/ConsultingAdmin/' + id;
                return httpGet(url);
            };
...
(function () {
    ...
    app.service('common', [function () {
        ...
        var getShowPages = function getShowPages(pageTotal, currentPage, pageSize) {
            var showPages = []; //显示的页码

            if (pageTotal > 0) {
                currentPage = currentPage || 1;
            }
            var showPageEnd = pageTotal;
            var showPageStart = 1;
            if (pageTotal > 5) {
                if (currentPage > 2) {
...
{
    'use strict';
    angular.module('healthAdmin').constant('healthAdminConstant', {
        pageSize: 10,
        gender: { //性别
            0: "",
            1: "男",
...
{
    'use strict';
    angular.module('healthAdmin').controller('consultingDetailCtr', ['$scope', '$stateParams', 'api', 'common', function ($scope, $stateParams, api, common) {
        var consultingId = $stateParams.id;

        var getDetail = function getDetail() {
            api.getConsulting(consultingId).then(function (data) {
                $scope.detail = data;
            });
        };
        ...
        var hideFaq = function hideFaq() {
            $scope.faq = null;
        };

        var showFaq = function showFaq(faq) {
            $scope.faq = faq;
        };
...
{
            'use strict';
            angular.module('healthAdmin').controller('consultingListCtr', ['$scope', '$timeout', 'healthAdminConstant', 'api', 'common', function ($scope, $timeout, healthAdminConstant, api, common) {

                        var changeTab = function changeTab(status) {
                                    $scope.filterData.status = status;
                                    searchConsultings();
                        };

                        var getConsulting = function getConsulting() {
                        ...
                        var searchConsultings = function searchConsultings(pageIndex) {
...

{
    ...
    angular.module('healthAdmin').directive('page', [function () {
        return {
            restirct: 'AE',
            replace: true,
            scope: {
            ...
            link: function link(scope, iElement, attr) {
                var getShowPages = function getShowPages(pageTotal, currentPage, pageSize) {
                    var showPages = []; //显示的页码
        ...
        return {
            ...
            link: function link(scope, iElement, attr) {
...

                {
                    ...
                    var currentWeekDay = _newDate.getDay() || 7; //周日用7表示
                    _newDate.setDate(_newDate.getDate() - currentWeekDay + 1);
                    scope.starttime = _newDate;
                    //console.log("linkstart:" + new Date() + JSON.stringify(scope));
                }

                var changeendtime = function changeendtime() {
                    scope.endtime = scope.endtime || new Date();
                    if (scope.endtime < scope.starttime) {
                        scope.endtime = scope.starttime;
                    }
                };
                var changeMinTime = function changeMinTime() {
                    if (scope.starttime) {
                        var mintime = scope.starttime;
...
(function () {
    ...
    var app = angular.module('healthAdmin');
    app.controller('coinDetailListCtr', ['$scope', "$stateParams", '$timeout', '$state', 'api', 'common', 'healthAdminConstant', function ($scope, $stateParams, $timeout, $state, api, common, healthAdminConstant) {
        //$stateParams 获取url参数
        var doctorId = $stateParams['doctorId'];
        var action = $stateParams['action'];

        var filterStatusStr = "&$filter=CoinDetailType eq '4'"; //默认是签到
        var getCoinDetails = function getCoinDetails() {
        ...
        var serchCoinDetails = function serchCoinDetails(pageIndex) {
            $scope.currentPage = pageIndex || 0;
            getCoinDetails();
        };
...
(function () {
    ...
    var app = angular.module('healthAdmin');
    app.controller('exchangeCtr', ['$rootScope', '$scope', '$state', '$stateParams', '$timeout', '$sce', 'api', 'common', 'healthAdminConstant', function ($rootScope, $scope, $state, $stateParams, $timeout, $sce, api, common, healthAdminConstant) {
        $scope.pageSize = healthAdminConstant.pageSize; //页码大小
        var action = $stateParams['action']; //review,pay ，all
        var selectedExchangeIds = [];
        ...
        var serchExchanges = function serchExchanges(pageIndex) {
            if (!pageIndex) {
                //search 
                $scope.selectedIds = {};
        ...
        var pay = function pay(e) {
            var ids = [];
            for (var key in $scope.selectedIds) {
...
(function () {
    ...
    angular.module('healthAdmin').controller('payCtr', ['$stateParams', '$scope', '$sce', 'api', 'common', 'healthAdminConstant', function ($stateParams, $scope, $sce, api, common, healthAdminConstant) {
        var id = $stateParams['id'];
        var batchPayment = localStorage.getItem(id);
        if (batchPayment) {
...
(function () {
    ...
    app.controller('payListCtr', ["$scope", "api", '$rootScope', '$timeout', 'common', 'healthAdminConstant', function ($scope, api, $rootScope, $timeout, common, healthAdminConstant) {
        ...
        var pageSize = healthAdminConstant.pageSize; //页码大小

        var getPayList = function getPayList() {
        ...
        var serchPayments = function serchPayments(pageIndex) {
            $scope.currentPage = pageIndex || 0;
            getPayList();
        };
...
(function () {
    ...
    angular.module('healthAdmin').controller('reviewCtr', ['$scope', '$stateParams', '$state', 'api', function ($scope, $stateParams, $state, api) {
        var id = $stateParams.id;
        ...
        var selectedReasonChange = function selectedReasonChange() {
...
(function () {
    ...
    angular.module('healthAdmin').controller('loginCtr', ['$rootScope', '$scope', '$window', '$state', 'api', function ($rootScope, $scope, $window, $state, api) {
...
(function () {
    ...
    var app = angular.module('healthAdmin');
    app.controller('orderCtr', ["$scope", "api", '$rootScope', '$timeout', 'common', 'healthAdminConstant', function ($scope, api, $rootScope, $timeout, common, healthAdminConstant) {
        ...
        var serchOrders = function serchOrders(pageIndex) {
            $scope.currentPage = pageIndex || 0;
            getOrderList();
        };
        var searchStatus = function searchStatus(status) {
            $scope.filterData.status = status;
            serchOrders();
        };
...
(function () {
    ...
    app.controller('patientListCtr', ["$scope", "api", '$rootScope', '$timeout', 'common', 'healthAdminConstant', function ($scope, api, $rootScope, $timeout, common, healthAdminConstant) {
        ...
        var searchPatient = function searchPatient(pageIndex) {
            $scope.currentPage = pageIndex || 0;
            getPatientList();
        };
